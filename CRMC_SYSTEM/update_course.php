...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['courseName'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM course WHERE courseName = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['updateCourse'])) {
    ...
    $stmt = $pdo->prepare("UPDATE course SET courseName = ? WHERE courseName = ?");
    $stmt->execute([$updateCourseName, $courseName]);
...
<body>
    ...
    <form method="POST" action="update_course.php" enctype="multipart/form-data">
        ...
        <label for="updateCourseName">Updated course Name:</label>
        <input type="text" name="updateCourseName" value="<?= $course['courseName']; ?>" required>
